...
let
  # Like linkFarm but copies packages instead of symlinking them. Zig
  # computes cwd-relative paths for build-time tool runs against the
  # entry path; with symlinks the kernel resolves them against the
  # link target instead and the tools fail to spawn (FileNotFound).
  copyFarm =
    name: entries:
    runCommandLocal name { } ''
      mkdir -p $out
      ${lib.concatMapStrings (entry: ''
        cp -R ${entry.path} $out/${entry.name}
      '') entries}
    '';
...
copyFarm name [
  ...
  {
    name = "clap-0.11.0-oBajBxDoAQChQCl4LRM4tyf-VChKLbhYilS806xxzxrQ";
    path = fetchZigArtifact {
      name = "clap";
      url = "git+https://github.com/Hejsil/zig-clap#27621cb7207643f914bae7b01902b22a8b5916e7";
      hash = "sha256-jqzzo1ma78M8lfLfdXHyzEsr6PEPQSigrF3Nx2EJ/LI=";
    };
  }
  {
  ...
  }
  {
    ...
    path = fetchZigArtifact {
      ...
      hash = "sha256-Jlg30wJrQz8Oa05J1DFTuRWwoZUT9+3YqOaTxVm9QVs=";
    };
  }
  {
